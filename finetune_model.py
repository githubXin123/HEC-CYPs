...
def main(template_path):
    ...
    for isoform in isoforms:
        ...
        model.cuda()
        weight_path = config.best_model_pts[isoform]
        model.load_state_dict(torch.load(weight_path))
        # freeze_mode: 'all'=冻全部(原fixed), 'encoders'=只冻UniMol+ESM-2(训注意力+MLP), 'none'=全参数
        freeze_mode = getattr(config, 'freeze_mode', 'all')
        if freeze_mode in ('all', 'encoders'):
            for param in model.molecule_encoder.parameters():
                param.requires_grad = False
            for param in model.protein_encoder.parameters():
                param.requires_grad = False
        if freeze_mode == 'all':
            for param in model.transformer_layer_cross_attention.parameters():
                param.requires_grad = False
            for param in model.transformer_layer_self_attention.parameters():
                param.requires_grad = False
        # 兼容旧的 fixed 参数
        if hasattr(config, 'fixed') and config.fixed and not hasattr(config, 'freeze_mode'):
            for param in model.transformer_layer_cross_attention.parameters():
                param.requires_grad = False
            for param in model.transformer_layer_self_attention.parameters():
                param.requires_grad = False
...
if __name__ == '__main__':
    ...
    parser.add_argument("-t", "--template", type=str, default="template/model_finetuning.json", help="input finetune template")
